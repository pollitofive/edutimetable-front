...
export interface CourseLevel { id: string; track: string; name: string; slug: string; sort_order: number }
...
const GET_COURSE_LEVELS = gql`
  query GetCourseLevels { courseLevels(first: 100, page: 1) { data { id track name slug sort_order } } }
`
const GET_STUDENTS = gql`
  query GetStudents($first: Int!, $page: Int!, $search: String, $course_level_id: ID, $track: String) {
    students(first: $first, page: $page, search: $search, course_level_id: $course_level_id, track: $track) {
      data { id name email phone course_level_id courseLevel { id track name slug sort_order } }
...
const CREATE_STUDENT = gql`
  mutation CreateStudent($input: CreateStudentInput!) {
    createStudent(input: $input) { id name email phone course_level_id courseLevel { id name track } }
  }
`
const UPDATE_STUDENT = gql`
  mutation UpdateStudent($id: ID!, $input: UpdateStudentInput!) {
    updateStudent(id: $id, input: $input) { id name email phone course_level_id courseLevel { id name track } }
  }
`
...
export function useStudents() {
  ...
  const hasActiveFilters = computed(() => filterSearch.value.trim() !== '' || filterTrack.value.trim() !== '' || filterCourseLevel.value.trim() !== '')
  const availableTracks = computed(() => [...new Set(courseLevels.value.map(l => l.track))].sort())
  const filteredCourseLevels = computed(() =>
    formState.value.selectedTrack ? courseLevels.value.filter(l => l.track === formState.value.selectedTrack).sort((a, b) => a.sort_order - b.sort_order) : []
  )
  const filteredCourseLevelsForFilter = computed(() =>
    filterTrack.value ? courseLevels.value.filter(l => l.track === filterTrack.value).sort((a, b) => a.sort_order - b.sort_order) : []
  )
  ...
  const closeModal = () => {
    ...
    formData.value = { name: '', email: '', phone: '', course_level_id: '' }
    formState.value = { selectedTrack: '' }; formErrors.value = {}
  }
  ...
  const clearFilters = () => { filterSearch.value = ''; filterTrack.value = ''; filterCourseLevel.value = ''; applyFilters() }
