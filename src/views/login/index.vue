...
const { t, email, password, rememberMe, isLoading, errors, handleLogin, handleKeyPress } = useLogin()
</script>
...
<template>
  <div class="container grid lg:h-screen grid-cols-12 lg:max-w-[1550px] 2xl:max-w-[1750px] py-10 px-5 sm:py-14 sm:px-10 md:px-36 lg:py-0 lg:pl-14 lg:pr-12 xl:px-24">
    <div :class="['relative z-50 h-full col-span-12 p-7 sm:p-14 bg-white rounded-2xl lg:bg-transparent lg:pr-10 lg:col-span-5 xl:pr-24 2xl:col-span-4 lg:p-0 dark:bg-darkmode-600', 'before:content-[\'\'] before:absolute before:inset-0 before:-mb-3.5 before:bg-white/40 before:rounded-2xl before:mx-5 dark:before:hidden']">
      <div class="relative z-10 flex flex-col justify-center w-full h-full py-2 lg:py-32">
        <div class="rounded-[0.8rem] w-[55px] h-[55px] border border-primary/30 flex items-center justify-center">
          <div class="relative flex items-center justify-center w-[50px] rounded-[0.6rem] h-[50px] bg-gradient-to-b from-theme-1/90 to-theme-2/90 bg-white">
        ...
        <div class="mt-10">
          ...
          <div class="mt-6">
            ...
            <FormInput v-model="email" type="email" class="block px-4 py-3.5 rounded-[0.6rem] border-slate-300/80" :class="{ 'border-red-500': errors.email }" placeholder="admin@example.com" @keypress="handleKeyPress" :disabled="isLoading" />
            <div v-if="errors.email" class="mt-1 text-xs text-red-500">{{ errors.email }}</div>

            <FormLabel class="mt-4">{{ t('login.password') }}*</FormLabel>
            <FormInput v-model="password" type="password" class="block px-4 py-3.5 rounded-[0.6rem] border-slate-300/80" :class="{ 'border-red-500': errors.password }" placeholder="************" @keypress="handleKeyPress" :disabled="isLoading" />
            ...
            <div class="flex mt-4 text-xs text-slate-500 sm:text-sm">
              <div class="flex items-center mr-auto">
                <FormCheck.Input id="remember-me" v-model="rememberMe" type="checkbox" class="mr-2.5 border" :disabled="isLoading" />
                <label class="cursor-pointer select-none" htmlFor="remember-me">{{ t('login.rememberMe') }}</label>
              </div>
              <a href="#" class="hover:text-primary">{{ t('login.forgotPassword') }}</a>
            </div>

            <div class="mt-5 text-center xl:mt-8 xl:text-left">
              <Button variant="primary" rounded class="bg-gradient-to-r from-theme-1/70 to-theme-2/70 w-full py-3.5 xl:mr-3 dark:border-darkmode-400" @click="handleLogin" :disabled="isLoading">
  ...
  <div class="fixed container grid w-screen inset-0 h-screen grid-cols-12 lg:max-w-[1550px] 2xl:max-w-[1750px] pl-14 pr-12 xl:px-24">
    ...
    <div :class="['h-full col-span-7 2xl:col-span-8 lg:relative', 'before:content-[\'\'] before:absolute before:lg:-ml-10 before:left-0 before:inset-y-0 before:bg-gradient-to-b before:from-theme-1 before:to-theme-2 before:w-screen before:lg:w-[800%]', 'after:content-[\'\'] after:absolute after:inset-y-0 after:left-0 after:w-screen after:lg:w-[800%] after:bg-texture-white after:bg-fixed after:bg-center after:lg:bg-[25rem_-25rem] after:bg-no-repeat']">
      <div class="sticky top-0 z-10 flex-col justify-center hidden h-screen ml-16 lg:flex xl:ml-28 2xl:ml-36">
        <div class="leading-[1.4] text-[2.6rem] xl:text-5xl font-medium xl:leading-[1.2] text-white">{{ t('login.hero.title') }}</div>
        <div class="mt-5 text-base leading-relaxed xl:text-lg text-white/70">{{ t('login.hero.description') }}</div>
        <div class="mt-10"><div class="text-base leading-relaxed text-white/70">{{ t('login.hero.tagline') }}</div></div>
      </div>
    </div>
  </div>
  <ThemeSwitcher />
